...
impl<'a> FromValue<'a> for CompressedScript {
    fn from_value(ctx: &FromValueContext<'a>, value: &Value) -> Result<Self, FromValueError> {
        ...
        let script_0_value: &Value = ctx.from_value(script[0])?;
        let id = match script_0_value.kind() {
            ValueKind::String => {
                let id: &StringValue = ctx.from_value(script[0])?;
                let id = std::str::from_utf8(id.value()).map_err(FromValueError::new_other)?;
                let id: i32 = id.parse().map_err(FromValueError::new_other)?;
                id
            }
            ValueKind::Fixnum => {
            ...
            kind => {
                return Err(ctx.new_unexpected_value_kind_error(kind));
            }
        };
...
impl IntoValue for CompressedScript {
    fn into_value(self, arena: &mut ValueArena) -> Result<ValueHandle, IntoValueError> {
        let id = arena
            .create_string(itoa::Buffer::new().format(self.id).into())
            .into();
